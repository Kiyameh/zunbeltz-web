...
describe("PostHeader", () => {
  describe("Rendering", () => {
    test("Should render post title", async () => {
      const html = await renderAstroComponent(PostHeader, {
        props: { post: mockPost },
      });
    ...
    test("Should render post description", async () => {
      ...
      expect(html).toContain(
        "This is a test post header description with more details",
      );
    });
  ...
  describe("Structure", () => {
    test("Should have correct header structure", async () => {
      ...
      expect(html).toMatch(
        /<header[^>]*class="post-header"[^>]*>.*<\/header>/s,
      );
    });
    ...
    test("Meta should contain time and categories", async () => {
      ...
      expect(html).toMatch(
        /<div[^>]*class="post-meta"[^>]*>.*<time.*<\/time>.*<div[^>]*class="post-categories".*<\/div>.*<\/div>/s,
      );
    });
    ...
    test("Categories should be in chip elements", async () => {
      ...
      expect(html).toMatch(
        /<span[^>]*class="chip secondary"[^>]*>Espeleología<\/span>/,
      );
      expect(html).toMatch(
        /<span[^>]*class="chip secondary"[^>]*>Aventura<\/span>/,
      );
      expect(html).toMatch(
        /<span[^>]*class="chip secondary"[^>]*>Naturaleza<\/span>/,
      );
    });
